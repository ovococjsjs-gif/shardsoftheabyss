...
function bodyParagraph(text, cfg, firstInChapter = false) {
  return new Paragraph({
    ...
    indent: { firstLine: firstInChapter && cfg.compactContents ? 0 : cfg.firstLine },
    widowControl: true,
...
function bodyFooter(cfg) {
  const children = !cfg.compactContents
...
async function buildChapterFiles(kind = "editorial") {
  const baseCfg = FORMATS[kind];
  if (!baseCfg) throw new Error(`Неизвестный формат: ${kind}`);
  const chapters = chapterFiles();
  const chapterDir = path.join(EXPORT_DIR, "chapters");
  fs.mkdirSync(chapterDir, { recursive: true });
  for (const stale of fs.readdirSync(chapterDir).filter((name) => /^Глава_\d+\.docx$/.test(name))) {
    fs.rmSync(path.join(chapterDir, stale));
  }

  const results = [];
  for (const chapter of chapters) {
    const cfg = { ...baseCfg, headerText: `${BOOK_TITLE} · Глава ${chapter.number}` };
    const body = parseChapter(chapter, cfg, true);
    const doc = new Document({
      creator: "",
      title: `${SERIES_TITLE} — ${BOOK_TITLE} — Глава ${chapter.number}`,
      subject: `Глава ${chapter.number}`,
      description: `${baseCfg.label}, отдельная глава`,
      keywords: "фэнтези, роман, Хроники Этериума, Осколки Бездны",
      features: { updateFields: true },
      styles: {
        default: {
      ...
      sections: [{
        properties: { page: { size: cfg.page, margin: cfg.margin, pageNumbers: { start: 1 } } },
        headers: { default: bodyHeader(cfg) },
        footers: { default: bodyFooter(cfg) },
        children: body,
      }],
    });
    const target = path.join(chapterDir, `Глава_${String(chapter.number).padStart(2, "0")}.docx`);
    const buffer = await Packer.toBuffer(doc);
    fs.writeFileSync(target, buffer);
    results.push({ target, bytes: buffer.length, paragraphs: body.length });
  }
  return results;
}
...
async function main() {
  ...
  if (args.includes("--chapters")) {
    for (const kind of kinds) {
      const results = await buildChapterFiles(kind);
      const bytes = results.reduce((sum, item) => sum + item.bytes, 0);
      console.log(`${kind} chapters: ${results.length} files, ${bytes} bytes total`);
    }
    return;
  }
  for (const kind of kinds) {
