...
for suffix,name in [('', 'Book'),('-Italic','BookI'),('-Bold','BookB'),('-BoldItalic','BookBI')]:
    pdfmetrics.registerFont(TTFont(name,str(ED.parent/'2026-09-16'/'fonts'/f'DejaVuSerif{suffix}.ttf')))
...
class BookDoc(BaseDocTemplate):
    def __init__(self,path):
        super().__init__(str(path),pagesize=(W,H),leftMargin=LEFT,rightMargin=RIGHT,
            ...
            author='',subject='Редакторская сборка 17 сентября 2026',pageCompression=1)
...
story=[Spacer(1,46*mm),Paragraph('ОСКОЛКИ<br/>БЕЗДНЫ',title),Spacer(1,10*mm),
       ...
       Paragraph('Редакторская сборка<br/>17 сентября 2026',small),PageBreak(),
...
# Record the PDF only after successful typesetting.
import hashlib
manifest_path=ED/'manifest.json'
manifest=json.loads(manifest_path.read_text())
manifest['outputs']['shards-of-the-abyss-edited.pdf']=hashlib.sha256((ED/'shards-of-the-abyss-edited.pdf').read_bytes()).hexdigest()
