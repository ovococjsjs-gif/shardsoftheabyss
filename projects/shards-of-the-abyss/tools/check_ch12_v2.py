...
R = P.parents[1]
BASE = '0d9dbac5884ada36499eafdd8ade808cede3eb8c'
BRANCH = 'arena/01a08101-workspace'
REPORT = P / 'review/ch-12-v2-check.json'
...
chapter = P / 'drafts/ch-12/ch-12-v2.md'
source = P / 'book/03-manuscript/arc-03/ch-12.md'
text = chapter.read_text()
assert text.startswith('# Глава 12\n\n') and text.endswith('\n')
...
pairs = json.loads((P / 'review/ch-12-before-after.json').read_text())
assert len(pairs) == 2
rendered = (P / 'review/ch-12-before-after.md').read_text()
verified = []
for pair in pairs:
    assert pair['before_path'] == 'book/03-manuscript/arc-03/ch-12.md'
    assert pair['after_path'] == 'drafts/ch-12/ch-12-v2.md'
...
refinements = json.loads((P / 'review/ch-12-v2-refinements.json').read_text())
assert len(refinements) == 53
reconstructed = text
...
initial_cache = R / '.cache/ch12/initial.md'
...
report = dict(
    ...
    comparison_baseline='September source chapter 12; no previously delivered new chapter 12 existed.',
    ...
    source_ch12=stats(source),
    new_ch12=stats(chapter),
    comparisons=verified,
    local_markdown_links_checked=links,
    formatting_checks_passed=True,
    contextual_refinement_operations=53,
    initial_draft_reconstructed_sha256=initial_hash,
    refinement_sequence_reverse_replayed=True,
    reading_boundary='All selected source 11–17 read in multiple stages; source 12 reread fully for this chapter. New 12 read fully twice; final 10 edits read in local context. Earlier PDF 17 not read. No new 13–17 prose.',
)
